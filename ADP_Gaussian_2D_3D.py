...
#for (u1, v1, w1) in [(0,0,1)]:
    ...
    input_directory = r"D:\___\New folder"
    output_directory = r"D:\___\New folder"    
        ...
        if filename.endswith(".cif"):
            ...
            ###########################################        
               ...
               # Calculating delta1 for each number in hkl_same and checking the condition
                ...
                output_filename = os.path.splitext(filename)[0] + f"_u{u1}_v{v1}_w{w1}_1D.xlsx"
                ...
                # Save the plot
                output_filename = os.path.splitext(filename)[0] + f"_u{u1}_v{v1}_w{w1}_Ring_3D_guss.png"
                output_filepath = os.path.join(output_directory, output_filename)
                ...
                # Save the plot
                output_filename = os.path.splitext(filename)[0] + f"_u{u1}_v{v1}_w{w1}_123.png"
                output_filepath = os.path.join(output_directory, output_filename)
                plt.savefig(output_filepath, bbox_inches='tight')  # Add bbox_inches='tight' for a tighter fit
                plt.close()    
                print(f"Saved plot: {output_filename}")
                ...
                # Create an Excel writer and save the DataFrame
                output_filename = os.path.splitext(filename)[0] + f"_u{u1}_v{v1}_w{w1}_2D_all.xlsx"
                output_filepath = os.path.join(output_directory, output_filename)
                with pd.ExcelWriter(output_filepath) as writer:
                    df.to_excel(writer, sheet_name="Matrix", index=False)
                    # No need to call writer.save() as it is handled by the context manager
