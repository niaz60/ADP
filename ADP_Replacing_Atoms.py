...
import random  # <-- Used for random selection of rows and elements
...
#################################################
# Wavelength
lambda_h = 1.5406  # Angstrom

repp = 0.01
##################################################
# Zone axis
for (u1, v1, w1) in [(1, 0, 0)]:  # You can modify or iterate over multiple zone axes
##################################################


    # Lattice constant from CIF
    input_directory = r"D:\___replace\____________________cif\cif"
    output_directory = r"D:\___replace\____________________cif\____1___100"
    ...
    for filename in os.listdir(input_directory):
        if filename.endswith(".cif"):
            ...
            try:
                ...
                # Extract 'a', 'b', 'c', 'alpha', 'beta', 'gamma' from CIF
                a = None
                b = None
                c = None
                alpha = None
                ...
                # Extract matrix from lines after the known keywords
                matrix_start = False
                matrix_data = []
                for line in lines:
                ...
                # Handle extraction of x, y, z, atom_names differently depending on whether "ICSD" or "www.crystallography.net" is in the file
                ...
                # Convert new_positions to NumPy array
                ...
                # Round numerical values to 7 decimal places
                for i in range(new_positions.shape[0]):
                    for j in range(1, new_positions.shape[1]):
                        try:
                ...
                # -----------------------------------------------------------
                # This is where you have your final new_positions array
                # -----------------------------------------------------------
                

                # --- ADDED CODE FOR RANDOM ATOM REPLACEMENT ---
                # Randomly select 60% of the rows in new_positions
                # and replace the first column with random elements

                # A sample list of Periodic Table elements:
                ...
                num_rows = new_positions.shape[0]
                num_to_replace = int( (repp) * num_rows)  # 60% of total rows
                random_indices = random.sample(range(num_rows), num_to_replace)

                for idx in random_indices:
                    # Pick a random element
                    random_element = random.choice(periodic_table_elements)
                    # For simplicity, attach "0+"
                    new_positions[idx, 0] = random_element
                # -----------------------------------------------------------

                # === THIRD PART OF YOUR CODE ===
                # fractional positions
                points1 = new_positions[:, 1:4]  # columns 1,2,3
                points = points1.astype(float)   # convert to float
                ...
                # Put the transformed names back into new_positions
                new_positions[:, 0] = atom_names.ravel()

                # (From here on, you might do additional calculations or save the file, etc.)
                
                # ----------------------------------------------------------
                # For demonstration, you could print the final new_positions:
                # print("Final new_positions:")
                # print(new_positions)
                # ----------------------------------------------------------
            ...
            ###########################################        
               ...
               # Calculating delta1 for each number in hkl_same and checking the condition
                ...
                points_transformed = rotate_points(points_transformed, 0)
                ...
                # Save the plot
                output_filename = os.path.splitext(filename)[0] + f"_u{u1}_v{v1}_w{w1}_{repp}.png"
                output_filepath = os.path.join(output_directory, output_filename)
